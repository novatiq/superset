...
describe('getValueFormatter in embedded context', () => {
  ...
  it('should format as EUR for German locale', () => {
    window.location.search = '?locale=de-DE';
    const formatter = getValueFormatter([], {}, {}, undefined, undefined);
    expect(formatter.format(12345.67)).toEqual('12.345,67 €');
  });

  it('should use custom symbol when provided', () => {
    window.location.search = '?locale=de-DE&currencySymbol=DM';
    const formatter = getValueFormatter([], {}, {}, undefined, undefined);
    expect(formatter.format(12345.67)).toEqual('12.345,67 DM');
  });

  it('should handle percentage format and ignore currency', () => {
    window.location.search = '?locale=en-US';
    const formatter = getValueFormatter([], {}, {}, '%.2f', undefined);
    expect(formatter.format(0.1234)).toEqual('12.34%');
  });

  it('should fall back to d3Format on invalid locale', () => {
    window.location.search = '?locale=xx-XX';
    const formatter = getValueFormatter([], {}, {}, ',.2f', undefined);
    expect(formatter).toBeInstanceOf(NumberFormatter);
    expect(formatter.format(12345.67)).toEqual('12,345.67');
  });

  it('should use fallback replacement if formatToParts is missing', () => {
    const originalFormatToParts = Intl.NumberFormat.prototype.formatToParts;
    // @ts-ignore
    delete Intl.NumberFormat.prototype.formatToParts;

    window.location.search = '?locale=en-US&currencySymbol=@';
    const formatter = getValueFormatter([], {}, {}, undefined, undefined);
    // native.format() for en-US gives '$12,345.67'
    // The regex should replace '$' with '@'
    expect(formatter.format(12345.67)).toEqual('@12,345.67');
